<script lang="ts">
	...
	const customFadeIn = (node: Element, options = undefined) =>
		fade(node, { delay: 300, duration: 300 });
...
>
	...
	<div class="col-span-full mt-3">
		{#if $validation.hasError("password.required") || $validation.hasError("password.min")}
			<p class="text-red-500 text-[.8rem] mb-1">
				Password field must contain atleast 8 characters
			</p>
		{/if}
		{#if $validation.hasError("confirm.required") || $validation.hasError("confirm.match_field")}
			<p class="text-red-500 text-[.8rem]">
				Confirm pasword field must be filled & matching
			</p>
		{/if}
	</div>
	...
	<button
		...
		type="button"
		class="col-span-2 text-center text-gray-600 text-sm">Sign In</button
	>
</AuthCard>
