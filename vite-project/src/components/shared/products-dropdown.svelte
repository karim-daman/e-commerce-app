...
<div class="col-start-2 col-end-3 relative">
  <button class="text-grey-secondary p-[6px] py-2 grid place-content-center rounded-[.25rem] border border-[rgba(81,88,94,.12)] hover:text-blue-primary mr-4" on:click={toggle}>
    <ProductsHamburger class="h-4 transform translate-y-[-3px]" />
  </button>

  {#if open}
    <DropdownBox --left={$isMobile && "auto"} --right={$isMobile && "16px"} {choices} bind:selected />
  {/if}
</div>
