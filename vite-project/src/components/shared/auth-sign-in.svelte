<script lang="ts">
  ...
  const submission = async () => {
    ...
    //   error = typeof errorRes.message === "string" ? [errorRes.message] : errorRes.message;
    // } else {
  ...
  const customFlyIn = (node: Element, options = undefined) => fly(node, { delay: 300, duration: 300, y: flyY });
  const customFadeOut = (node: Element, options = undefined) => fade(node, { duration: 300 });
</script>

<AuthCard class="py-12 px-8 grid grid-cols-2 grid-flow-row gap-x-4 auto-rows-max" on:submit={submission} transIn={customFlyIn} transOut={customFadeOut}>
  ...
  <div class="col-span-2 mt-3">
    ...
    <p class="text-[.8rem] mb-2 text-red-500">Password must be minimum of 8 character</p>
    <!-- {/if} -->
    <!-- <AuthPasswordInput bind:value={$password.value} isInvalid={$password.invalid} placeholder="Password"> -->
  ...
  <p class="col-span-2 text-center text-gray-600 text-sm mt-7">Don't Have an account?</p>
  <button on:click={handleSignUp} type="button" class="col-span-2 text-center text-gray-600 text-sm">Sign Up</button>
</AuthCard>
