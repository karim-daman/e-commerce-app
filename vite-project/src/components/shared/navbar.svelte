...
<nav class="grid grid-flow-rows auto-rows-max w-full">
	<div class="w-full px-5 lg:px-20 bg-[#f8f9fa] grid">
		<div
			class="grid justify-self-center max-w-[1320px] grid-cols-[max-content,1fr,repeat(5,max-content)] w-full gap-[15px] py-2 grid-rows-1"
		>
			<NavbarHeadDropdown
				choices={languages}
				bind:selected={selected.languages}
			>
				Language
			</NavbarHeadDropdown>
			<NavbarHeadDropdown
				...
				choices={currency}
				bind:selected={selected.currency}
			>
			...
			{#if !$session.user}
				<button
					on:click={handleModalSigninToggle}
					class="!hidden md:!block">Authenication</button
				>
			{/if}

			<Modal bind:open={modalSigninToggle} class="z-20 grid ">
				<Signin
					bind:flyY={modalSignInFly}
					handleSignUp={handleModalRegisterToggle}
				/>
			</Modal>
			...
			{#if $session.user}
				<button on:click={handleSignout} class="!hidden md:!block"
					>Sign out</button
				>
			{:else}
				<button
					on:click={handleModalSigninToggle}
					class="!hidden md:!block">Sign in</button
				>
				<button
					on:click={handleModalRegisterToggle}
			...
			<button
				...
				class="col-start-[-2] col-end-[-1] md:!hidden"
			>
	...
	{#if isDrawerOpen}
		<div transition:slide class="w-full px-5">
			{#if !$session.user}
				<button class="mt-[15px]" on:click={handleModalSigninToggle}>
					Authenication
				</button>
			{/if}
			...
			{#if $session.user}
				<button class="my-[15px]" on:click={handleSignout}
					>Sign out</button
				>
			{:else}
				<button on:click={handleModalSigninToggle} class="mt-[15px]">
					Sign in
				</button>
				<button on:click={handleModalRegisterToggle} class="mb-[15px]">
					Register
				</button>
			{/if}
		</div>
	...
	<div class="w-full px-5 lg:px-20 grid border border-[#dee2e6]">
		<div
			class="grid items-center gap-5 grid-cols-2 lg:grid-rows-1 lg:grid-cols-[repeat(2,minmax(0,max-content)),minmax(0,590px),minmax(0,1fr)] py-[15px] justify-self-center w-full max-w-[1320px]"
		>
			<a href="/">
				<img class="w-[160px]" src="/logo.png" alt="" />
			</a>

			<NavbarMainCategories
			...
			<a
				class="relative !hidden p-[10px] lg:!grid text-black/50 place-items-center rounded-full border border-black/50 justify-self-end bg-blue-primary hover:bg-blue-700"
				href="/cart"
			>
				<span
					class="absolute -top-1 left-[26px] bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full"
					>+99</span
				>
	...
	<div class="w-full lg:px-20 hidden lg:grid border-b border-[#dee2e6]">
		<div
			class="w-full max-w-[1320px] grid grid-flow-col gap-4 py-[15px] auto-cols-max justify-self-center"
		>
			<a
				class="!text-[15px] !text-black/80 hover:underline"
				href="/products">Products</a
			>
			<a class="!text-[15px] !text-black/80 hover:underline" href="/"
				>Testing page</a
			>
			<a class="!text-[15px] !text-black/80 hover:underline" href="/"
				>Admin Dash</a
