<script lang="ts">
  ...
  const handleRate = (id: number) => (event: MouseEvent) => {
    if (rating && event.target instanceof HTMLElement && rating.toString() === event.target.dataset.starid) {
      return;
    }
    rating = id;
  };
...
<div class="flex items-center my-2">
  ...
  <button on:click={() => (rating = 0)} type="button" class="rounded-btn-shadow bg-red-500 text-white mx-2 rounded border-2 border-black">
    <CancelRating />
  </button>
</div>
