...
<div class="grid gap-x-2 col-span-full sm:col-auto grid-cols-1 sm:grid-cols-[max-content,1fr]">
  <img class="sm:row-span-full justify-self-center sm:w-[72px] sm:h-[72px]" {src} alt="" />
  <div class="grid grid-cols-[minmax(0,1fr),minmax(0,max-content)] sm:grid-cols-none grid-flow-row auto-rows-max">
    <a class="underline font-normal text-[1rem] text-black" href="/">{title}</a>
...
<div class="w-full h-min grid grid-cols-[minmax(0,max-content),minmax(0,1fr),minmax(0,max-content)] gap-2">
  <button use:hold={dec} class="rounded-btn-shadow text-left p-1 border hover:text-blue-primary rounded-[.25rem]">
    <Minus />
  </button>
  <input class="border self-center sm:self-start border-black rounded-[.25rem] h-full sm:h-min p-1 px-3 transition-colors" bind:value={quantity} type="number" {min} />
  <button use:hold={inc} class="rounded-btn-shadow text-left hover:text-blue-primary p-1 border rounded-[.25rem] transition-colors">
    <Plus />
  </button>
</div>
