<script>
  ...
  import { app_categories } from "$stores/dataStore";
...
<div class=" flex flex-col col-span-full md:col-auto row-span-full">
  <Accordion multiple>
    <AccordionItem open="true">
      ...
      <div class="w-full border flex flex-col h-64 overflow-y-auto p-4">
        {#each $app_categories as category}
          <div class="grid gap-2 items-center grid-cols-[repeat(2,max-content),1fr]">
            <input class="w-[1rem] h-[1rem] border rounded-none" id="brands-{category.name}" bind:checked={category.selected} type="checkbox" />
      ...
      <Button class="w-full mt-5 transition hover:-translate-y-1 hover:shadow-lg active:shadow-none active:transform active:translate-y-0">Clear selection</Button>
    </AccordionItem>
    <AccordionItem open="true">
